...
> Durum: **Taslak** · MVP kapsamında · Bağımlılık: Modül 1, 2
...
| POST | /opponent-listings | Rakip arama ilanı (benzer yapı) |

## Veri Modeli
`matches` (id, public_id, team_id, opponent_team_id?, venue_text, venue_location
POINT?, starts_at UTC, format, price_per_player, status) ·
`match_participants` (match_id, user_id, source: team|listing, rsvp, responded_at) ·
`player_listings` (id, match_id, positions_needed JSON, level_min, level_max,
location POINT SPATIAL INDEX, status, expires_at) ·
...
- [ ] Keşif sorgusu: 10 km yarıçap + filtrelerle < 200ms (spatial index doğrula)
